    ...
    try:
        ...
        with open(file_name, newline="") as csv_file:
            ...
            if (row['AI Opp']):
                ...
                if (row['Wayfinder Opps']):  # Wayfinder Opp exists if "1" is chosen from AI Opp
                    eir_role2 = row['Wayfinder Opps'].split(",")
            ...
            if (row['Organization 1_1']):
                r = requests.get(
                    insightlyAPIurl + '/Organisations/Search?field_name=ORGANISATION_NAME&field_value=' + row[
                        'Organization 1_1'] + '&brief=false&count_total=false', auth=(insightlyAPIkey, ''))
            ...
            if (row['Organization 1_2']):
                ...
                json_dict2['CUSTOMFIELDS']['FIELD_VALUE'] = row['Organization 1_2']
            ...
            if (row['Q100']):
                ...
                json_dict2['CUSTOMFIELDS']['FIELD_VALUE'] = gender_decoder[int(row['Q100'])]
                json_dict2['CUSTOMFIELDS']['CUSTOM_FIELD_ID'] = 'Gender__c'
                json_dict['CUSTOMFIELDS'].append(json_dict2['CUSTOMFIELDS'])
            if (row['Q102']):
                ethnicity = []
                ethnicity.extend(row['Q102'].split(","))
